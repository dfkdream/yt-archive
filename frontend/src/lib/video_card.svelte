...
<Block strong inset class={needStyle?"!my-1 md:!my-4":""}>
    ...
    <h2 class={
        "mb-2 text-sm font-bold"+ (fullTitle ? "" : " whitespace-nowrap break-all truncate")
    }>{video.Title}</h2>

    {#if showChannel}
    <a href={`/channels/${video.Owner}`} class="flex items-center">
        <img src={`/api/channels/${video.Owner}/${video.OwnerThumbnail}`} alt={video.Owner} width="45px" class="rounded-full"/>
        <span class="text-sm mx-2">
            {video.Owner}
            <br />
            {video.Timestamp.toLocaleString([], {dateStyle: "medium", timeStyle: "medium"})}
        </span>
    </a>
    {:else}
        <div class="text-sm">
