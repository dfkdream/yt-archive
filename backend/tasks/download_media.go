...
func DownloadMediaHandler(task *taskq.Task) error {
	...
	tempMpdPath := filepath.Join(tempDir, "manifest.mpd")

	err = BuildManifest(payload.OutputPath, tempMpdPath)
	if err != nil {
		return err
	}

	tempMpd, err := mpd.FromFile(tempMpdPath)
	if err != nil {
		return err
	}

	masterMpd, err := mpd.FromFile(manifestPath)
	if err != nil {
	}

	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return tempMpd.WriteFile(manifestPath)
		}
		return err
	}

	return mpd.Merge(masterMpd, tempMpd).WriteFile(manifestPath)
}
